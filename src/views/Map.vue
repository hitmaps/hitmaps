    ...
    <div>
        ...
        <div v-if="mission != null" class="content">
            <div class="floor-toggle" v-if="mission.missionType != 'Sniper Assassin'">
                <div v-for="i in range(mission.lowestFloorNumber, mission.highestFloorNumber).reverse()" :key="i"
                     class="floor-info" :class="{'selected': currentLayer === i}">
            ...
            <l-map id="map" ref="map" @click="addMarker" :minZoom="3" :maxZoom="5"
                   :maxBounds="[[this.mission.topLeftCoordinate.split(',')[0], this.mission.topLeftCoordinate.split(',')[1]], [this.mission.bottomRightCoordinate.split(',')[0],this.mission.bottomRightCoordinate.split(',')[1]]]"
                   :crs="crs" @pm:drawstart="initDraw" @pm:create="pmLayer" @pm:drawend="endDraw">
                <l-tile-layer v-for="floor in range(mission.lowestFloorNumber, mission.highestFloorNumber)" :key="floor"
                              :noWrap="true" :visible="currentLayer === floor"
                              :url="mapUrl + floor +  '/{z}/{x}/{y}.png'"></l-tile-layer>
                <l-tile-layer v-if="mission.satelliteView" :noWrap="true" :visible="currentLayer === -99"
                              :url="mapUrl + '-99/{z}/{x}/{y}.png'"></l-tile-layer>
                <div v-for="floor in range(mission.lowestFloorNumber, mission.highestFloorNumber)"
                     :key="'layer' + floor">
                    <l-layer-group v-for="disguise in disguises" :key="floor + disguise.name"
                                   :visible="currentLayer == floor && editor.currentDisguise == disguise.id">
                        <l-polygon v-for="item in disguise.areas.filter(el=>el.level==floor)" :key="item.id"
                                   :lat-lngs="parseCoords(item.vertices)"
                                   :fillColor="item.type === 'trespassing' ? 'yellow' : '#f00'" :stroke="false"
                                   :weight="4" :opacity=".75">
                        </l-polygon>
                    </l-layer-group>
                    <l-layer-group v-for="(group, key) in layerGroups" :key="floor + key"
                                   :visible="currentLayer == floor && !isLayerHidden(key)">
                        <div v-if="group.name === 'Ledge'">
                            <l-polyline v-for="ledge in ledges.filter(el=>el.level==floor)" :key="ledge.id" color="#fff"
                                        :weight="4" :opacity=".75" :lat-lngs="parseCoords(ledge.vertices)">
                            </l-polyline>
                        </div>
                        <div v-else-if="group.name === 'Foliage'">
                            <l-polygon v-for="item in foliage.filter(el=>el.level==floor)" :key="item.id"
                                       color="#248f24" fillColor="#248f24" :weight="4" :opacity=".75"
                                       :lat-lngs="parseCoords(item.vertices)">
                            </l-polygon>
                        </div>
                        <div v-else>
                            <l-marker v-for="item in group.items.filter(el=>el.level==floor)" :key="item.id"
                                      :latLng="item.latLng" :draggable="editor.mode === 'items'"
                                      @dragend="moveMarker($event, item)">
                                <l-icon :icon-url="'/img/map-icons/' + item.icon + '.png'" :icon-size="[32, 32]"
                                        :icon-anchor="[16, 16]"
                                        :class-name="isSearchResult(item) ? 'search-result' : ''"></l-icon>
                                <l-popup>
                                    <img v-if="item.image" :src="'/img/png/' + item.image + '.png'"
                                    ...
                                    <div class="btn-group">
                                        <button class="btn btn-danger btn-sm" data-action="delete-btn"
                                                @click="deleteMarker(item)" data-toggle="tooltip" title="Delete">
                                            <i class="fas fa-times"></i>
                                        </button>
                                        <button class="btn btn-warning btn-sm" data-action="edit-btn"
                                                @click="editMarker(item)" data-node-id="x" data-toggle="tooltip"
                                                title="Edit">
            ...
            <nav class="navbar navbar-fixed-right navbar-dark">
                ...
                <div class="collapse navbar-collapse" id="navbarSupportedContent">
                    ...
                    <div class="accordion" id="accordion" v-show="!editor.enabled">
                        <div class="floor-toggle" v-if="mission.missionType != 'Sniper Assassin'">
                            <div v-for="i in range(mission.lowestFloorNumber, mission.highestFloorNumber).reverse()"
                                 :key="i" class="floor-info" :class="{'selected': currentLayer === i}">
                                <div @click="changeLevel(i)" class="floor">
                                    Level {{ i }}
                                </div>
                                <div :class="{'has-search-results': hasSearchResults(i)}" class="item-count">
                        ...
                        <div class="search-box" id="search-box-items" data-search="items">
                            <select @change="searchItem" ref="itemSearch" name="search" class="selectpicker"
                                    data-live-search="true" data-title="<i class='fas fa-fw fa-search'></i> Search"
                                ...
                                <template v-for="(type, key) in searchableNodes">
                                    <optgroup v-for="group in type.items" :key="key + group.name" :label="group.name">
                                        <option v-for="node in group.items" :key="node.name"
                                                :data-layer="key + '|' + group.name "
                                                :value="key + '|' + group.name + ';' + node.name">{{ node.name }}
                                        </option>
                                    </optgroup>
                                </template>
                            </select>
                            <button @click="clearSearch" id="clear-search" class="btn control-button"
                                    data-toggle="tooltip" title="Clear Search" v-show="searchedItem != null"><i
                                    class="fas fa-times"></i></button>
                        </div>
                        <div v-if="mission.missionType != 'Sniper Assassin'" class="search-box"
                             id="search-box-disguises" data-search="disguises">
                            <div class="card">
                                <div class="card-header" id="header-disguises">
                                    <div class="name collapsed control-button" data-toggle="collapse"
                                         data-target="#body-disguises" aria-expanded="false"
                              ...
                              <i class="fas fa-caret-up"></i>
                                ...
                                <div id="body-disguises" class="collapse" aria-labelledby="header-disguises">
                                    <div class="card-body disguises">
                                        <div @click="changeDisguise('NONE')" data-disguise-id="NONE"
                                             class="full-width selected"
                                             style="background: url('/img/jpg/disguises/none.jpg'); ">
                                            <p class="disguise-info">None</p>
                                        </div>
                                        <div @click="changeDisguise(disguise)" :data-disguise-id="disguise.id"
                                             v-for="disguise in disguises" :key="disguise.id" class="full-width"
                                             :style="{background: 'url(/img/jpg/' + disguise.image + '.jpg)'}">
                                            <p class="disguise-info">{{ disguise.name }}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <button id="clear-disguise-search" class="btn control-button" data-toggle="tooltip"
                                    title="Clear Search" style="display: none"><i class="fas fa-times"></i></button>
                        </div>
                        <div class="hide-or-select-all">
                            <button id="hide-all" @click="toggleLayer('|', true)" class="btn control-button"><i
                                    class="far fa-fw fa-eye-slash"></i> Hide All
                            </button>
                            <button id="show-all" @click="toggleLayer('|', false)" class="btn control-button"><i
                                    class="far fa-fw fa-eye"></i> Show All
                            </button>
                        </div>
                        <div v-if="mission.missionType == 'Sniper Assassin'">
                            <div v-for="(type, key, index) in nodes" :key="index" class="card">
                                <div class="card-body">
                                    <div v-show="group.items.length > 0" v-for="group in type.items"
                                         :key="type.name + group.name"
                                         :class="{'full-width': group.collapsible, 'half-width': !group.collapsible}">
                                        <div class="name" @click="toggleLayer(type.name + '|' + group.name)"
                                             :class="{'map-hidden': isLayerHidden(type.name + '|' + group.name)}">
                                            <img :src="'/img/map-icons/' + group.icon + '.png'"
                                                 :alt="group.name + ' Icon'" class="img-fluid">
                                            <span> {{group.name}}</span>
                                        </div>
                                        <div v-if="group.collapsible" class="visibility-toggle collapsed"
                                             data-toggle="collapse"
                                             :data-target="'#collapsible-' + collapsible(type, group)">
                                            <i class="fas fa-caret-up"></i>
                                            <i class="fas fa-caret-down"></i>
                                        </div>
                                        <div v-if="group.collapsible" class="collapsible-items collapse"
                      ...
                      <i class="fas fa-caret-up"></i>
                                    ...
                                    <div class="visibility-toggle group-toggle" @click="toggleLayer(type.name + '|*')"
                                ...
                                <div :id="'body-' + index" class="collapse" :aria-labelledby="'header-' + index">
                                    <div class="card-body">
                                        <div v-for="group in type.items" :key="type.name + group.name"
                                             :class="{'full-width': group.collapsible, 'half-width': !group.collapsible}">
                                            <div class="name" @click="toggleLayer(type.name + '|' + group.name)"
                                                 :class="{'map-hidden': isLayerHidden(type.name + '|' + group.name)}">
                                                <img :src="'/img/map-icons/' + group.icon + '.png'"
                    ...
                    <div class="edit-menu" v-show="editor.enabled && editor.mode === ''">
                        ...
                        <div v-if="mission.missionType != 'Sniper Assassin'" class="editor-button"
                    ...
                    <div class="items-menu" v-show="editor.enabled && editor.mode === 'items'">
                        ...
                        <p><i class="fas fa-fw fa-trash"></i> Click on an existing item and then the "Delete" button to
                            delete it.</p>
                    ...
                    <div class="ledges-menu" v-show="editor.enabled && editor.mode === 'ledges'">
                        <p data-ledge="delete-help"><i class="fas fa-trash"></i> Click on an existing ledge to delete
                            it.</p>
                    ...
                    <div class="foliage-menu" v-show="editor.enabled && editor.mode === 'foliage'">
                        <p data-foliage="delete-help"><i class="fas fa-trash"></i> Click on an existing foliage to
                            delete it.</p>
                        ...
                        <div class="editor-button"
                             @click="editorMenu(''); $refs.map.mapObject.pm.disableDraw('Polygon')">
                    ...
                    <div class="disguise-trespassing-menu" v-show="editor.enabled && editor.mode === 'disguises'">
                        <p data-disguise="delete-help"><i class="fas fa-trash"></i> Click on an existing region to
                            delete it.</p>
                        <p>Select a disguise from the dropdown below to edit its layout.</p>
                        <div class="search-box">
                            <select ref="disguisePicker" @change="editor.currentDisguise = $event.target.value"
                                    name="disguise-menu-dropdown" class="selectpicker" data-style="control-button">
                                <option value="NONE">None</option>
                                <option v-for="disguise in disguises" :key="disguise.id" :value="disguise.id">{{
                                    disguise.name }}
                                </option>
                            </select>
                        </div>
                        <div class="editor-button" data-disguise="add" data-type="trespassing"
                             @click="toggleDraw('Polygon'); editor.disguiseType = 'trespassing'"
                        ...
                        <div class="editor-button" data-disguise="add" data-type="hostile"
                             @click="toggleDraw('Polygon'); editor.disguiseType = 'hostile'"
                             v-if="editor.currentDisguise != 'NONE'">
                        ...
                        <div class="modal-body">
                            <div v-show="mission.missionType != 'Sniper Assassin'">
                                <div class="alert alert-primary" style="font-size: .8em">
                                    ...
                                    <b>If you select a template, all information in "Item Info" and "Notes" will be
                                        replaced with
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-10">
                                        <select @change="applyTemplate" name="template" ref="templatePicker"
                                                class="form-control selectpicker" title="Select One"
                                                data-live-search="true">
                                            <optgroup v-for="(group, key) in editor.templates" :key="key" :label="key">
                                                <option v-for="item in group" :key="item.id"
                                                        :value="key + '|' + item.id">{{item.name}}
                                                </option>
                                            </optgroup>
                                        </select>
                                    </div>
                            ...
                            <div class="alert alert-dark" style="font-size: .8em">
                                ...
                                <ul>
                                    <li>Any <b>required</b> items, such as needing a wrench for loosening valves (use
                                        type "Requirement")
                                    </li>
                                    <li>Whether or not the action is <b>suspicious</b> when wearing a certain disguise
                                        (use type "Warning")
                                    </li>
                                    <li>If you feel something <b>requires more information</b>, use the "Information"
                                        type
                                    </li>
                                    <li>You may add the in-game description for items by using the "Description" type
                                        <b>if there is no name field provided.</b></li>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-10">
                                    <select @change="selectSubgroup" name="subgroup" ref="subgroupPicker"
                                            class="form-control selectpicker" title="Select One"
                                            data-live-search="true">
                                        <optgroup v-for="(category, key) in categories" :key="key" :label="key">
                                            <option v-for="group in category" :key="group.id"
                                                    :value="key + '|' + group.subgroup"
                                                    :data-content="`<img height='24' width='24' src='/img/map-icons/` + group.icon + `.png' alt='` + group.group + ` Icon'> ` + group.group">
                                                {{group.group}}
                                            </option>
                                        </optgroup>
                                    </select>
                                    <small v-if="editor.currentCategory" class="form-text text-muted"
                                           id="note-help-text">
                            ...
                            <div v-show="pickIconAllowed" class="form-group row" id="icon-form-group">
                                ...
                                <div class="col-sm-10">
                                    <select name="icon" @change="selectIcon" ref="iconPicker"
                            ...
                            <div v-if="editor.currentCategory">
                                <div class="form-group row" v-if="currentCategory.requireName">
                                    ...
                                    <div class="col-sm-10">
                                        <input type="text" name="name" v-model="currentCategory.name"
                                               class="form-control">
                                ...
                                <div class="form-group row" v-if="currentCategory.requireAction">
                                    ...
                                    <div class="col-sm-10">
                                        <input type="text" name="action" v-model="currentCategory.action"
                                               class="form-control">
                                        <small class="form-text text-muted">
                                            Enter the action that is performed (such as "loosen the valve", "turn on
                                            radio", "trigger fire alarm", etc.)
                                        </small>
                                    </div>
                                </div>
                                <div class="form-group row" v-if="currentCategory.requireTarget">
                                    ...
                                    <div class="col-sm-10">
                                        <input type="text" name="target" v-model="currentCategory.target"
                                               class="form-control">
                                        <small class="form-text text-muted">
                                    ...
                                    <div class="col-sm-10">
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio"
                                                   v-model="currentCategory.pickupType" name="pickup-type" value="large"
                                                   checked>
                                        ...
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio"
                                                   v-model="currentCategory.pickupType" name="pickup-type"
                                                   value="small">
                                    ...
                                    <div class="col-sm-10">
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio"
                                                   ...
                                                   name="stairwell-direction" value="up-stair" checked>
                                        ...
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio"
                                                   v-model="currentCategory.stairwellDirection"
                                                   name="stairwell-direction" value="up-down-stair">
                                        ...
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio"
                                                   v-model="currentCategory.stairwellDirection"
                                                   name="stairwell-direction" value="down-stair">
                            ...
                            <div id="suggest-notes">
                                <div v-for="note in editor.notes" :key="note.type">
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-10">
                                            <input type="text" name="note-text[]" v-model="note.text"
                                                   class="form-control">
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="form-group row">
                                <div class="col-sm-8 offset-sm-2">
                                    <button type="button" @click="editor.notes.push({})" id="add-note-button"
                                            class="btn btn-dark">+ Add Another Note
                                    </button>
                                </div>
                            </div>
                        </div>
    ...
    export default {
        ...
        methods: {
            ...
            applyTemplate: function (event) {
                ...
                if (currentTemplate.description) this.editor.notes.push({
                    type: "description",
                    text: currentTemplate.description
                })
                if (currentTemplate.requirement) this.editor.notes.push({
                    type: "requirement",
                ...
                if (currentTemplate.information) this.editor.notes.push({
                    type: "information",
